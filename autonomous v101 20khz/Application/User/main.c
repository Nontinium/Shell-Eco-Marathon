...
uint8_t DEBOUNCE_THRESHOLD = 100;

float_t pot_software = 0;
float_t pot_hardware = 0;
float_t pot = 0;
float_t pot_adjusted;

float_t raw_min = 900;
float_t raw_max = 3800;
float_t rpm_low = 100;
float_t rpm_max = 2600;


uint16_t checks_driver = 0;
uint16_t checks_speed = 0;
uint16_t checks_curr = 0;
...
uint8_t RxData[5];         // Buffer for full messages after calibration
uint8_t singleByte = 0;    // Buffer to detect initial calibration byte
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    ...
    if (!is_calibrated) {
        // Calibration mode: Wait for single byte `;`
        if (singleByte == ';') {
            is_calibrated = true;
...
//            MC_ProgramSpeedRampMotor1_F(pot, 0);
        }
    } else {
